...
if ($result->num_rows > 0) {
    ...
    echo "<tr><th>Tipo de Servicio</th><th>Fecha y Hora</th><th>Mascota</th><th>Monto</th><th>Pagado</th></tr>";
    while ($turno = $result->fetch_assoc()) {
        ...
        echo "<tr><td><a href='detalle_turno.php?id_servicio=".$turno['id_servicio'] . "'>" . htmlspecialchars($turno['tipo_de_servicio']) . "</a></td><td>" . htmlspecialchars($turno['horario']) . "</td><td>" 
                . htmlspecialchars($nombre_mascota) . "</td><td>". htmlspecialchars($turno['monto']) . "</td><td>". htmlspecialchars($pagado) . "</td></tr>";
    }
    echo "</table>";
} else {
